...
static const uint8_t num_DSM_channels =
    6; // If using DSM RX, change this to match the number of
// 																					 transmitter
// channels you have
...
// Ring buffer for filetype FsFile (The filemanager that will handle the data
// stream)
...
unsigned long channel_8_fs =
    1000; // Perform step in pitch or roll (No step commands)
...
float B_accel = 0.14;    // Accelerometer LP filter paramter, (MPU6050 default:
                         // 0.14. MPU9250 default: 0.2)
float B_gyro = 0.1; // Gyro LP filter paramter, (MPU6050 default: 0.1. MPU9250
                    // default: 0.17)
float B_mag = 1.0;  // Magnetometer LP filter parameter

float cutoffFreq_pFilter =
    500; // Cutoff frequency for filtering the p-term in the PID calculation
...
// Pitch damping term for controlANGLE2(), lower is more damping (must be
// between 0 to 1)
...
int channel_1_pwm, channel_2_pwm, channel_3_pwm, channel_4_pwm, channel_5_pwm,
    channel_6_pwm, channel_7_pwm, channel_8_pwm, channel_9_pwm, channel_10_pwm,
    channel_11_pwm, channel_12_pwm, channel_13_pwm, channel_14_pwm;
...
float s1_command_scaled, s2_command_scaled, s3_command_scaled,
    s4_command_scaled, s5_command_scaled, s6_command_scaled, s7_command_scaled;
...
float sineTime =
    0.0f; // Counter used to determine time in the sine functions (seconds)
...
void setup() {
  ...
  //  m1_command_PWM = 125; //Command OneShot125 ESC from 125 to 250us pulse
  //  length m2_command_PWM = 125; m3_command_PWM = 125; m4_command_PWM = 125;
  //  m5_command_PWM = 125;
  //  m6_command_PWM = 125;
  ...
  while (channel_5_pwm > 1500) {
    ...
    getCommands();
    // Arm servo channels
    servo1.write(0); // Command servo angle from 0-180 degrees (1000 to 2000
                     // PWM)
    servo2.write(0); // Set these to 90 for servos if you do not want them to
                     // briefly max out on startup
    servo3.write(0); // Keep these at 0 if you are using servo outputs for
                     // motors
    servo4.write(0);
...
void loop() {
  ...
  // ONE: Prints radio pwm values (expected: 1000 to 2000)
  // printRadioData();
  //  Prints desired vehicle state commanded in either degrees or deg/sec
  //  (expected: +/- maxAXIS for roll, pitch, yaw; 0 to 1 for throttle)
  //printDesiredState();
  // Prints filtered gyro data direct from IMU (expected: ~ -250 to 250, 0 at
  // rest)
  // printGyroData();
  //  Prints filtered accelerometer data direct from IMU (expected: ~ -2 to 2;
  //  x,y 0 when level, z 1 when level)
  // printAccelData();
  //  Prints filtered magnetometer data direct from IMU (expected: ~ -300 to
  //  300)
  // printMagData();
  //  Prints roll, pitch, and yaw angles in degrees from Madgwick filter
  //  (expected: degrees, 0 when level)
  // printRollPitchYaw();
  //  Combines printRollPitchYaw() with printDesiredState() and prints in tab
  //  separted values in the order specified in the function.
  // printRollPitchYawAndDesired();
  //  Prints computed stabilized PID variables from controller and desired
  //  setpoint (expected: ~ -1 to 1)
  // printPIDoutput();
  ...
  // printServoCommands();
  //  Prints the time between loops in microseconds (expected: microseconds
  ...
  if ((channel_6_pwm < 1750) || extremeAngleFlag) {
	...
	// Check whether to set/reset the extremeAngleFlag
  ...
  // Get vehicle state
  getIMUdata(); // Pulls raw gyro, accelerometer, and magnetometer data from IMU
                // and LP filters to remove noise
  Madgwick(
      GyroX, -GyroY, -GyroZ, -AccX, AccY, AccZ, MagY, -MagX, MagZ,
      dt); // Updates roll_IMU, pitch_IMU, and yaw_IMU angle estimates (degrees)
...
void controlMixer() {
  ...
  /*
         * Takes roll_PID, pitch_PID, and yaw_PID computed from the PID
   controller and appropriately mixes
         * them for the desired vehicle configuration. For example on a
   quadcopter, the left two motors
         * should have +roll_PID while the right two motors should have
   -roll_PID. Front two should have
         * -pitch_PID and the back two should have +pitch_PID etc... every motor
   has normalized (0 to 1)
         * thro_des command for throttle control. Can also apply direct
   unstabilized commands from the
         * transmitter with
         * roll_passthru, pitch_passthru, and yaw_passthu. mX_command_scaled and
   sX_command scaled
         * variables are used in scaleCommands() in preparation to be sent to
   the motor ESCs and servos.
   *
   ...
   *channel_6_pwm - free auxillary channel, can be used to toggle things with an
         'if' statement
   */
...
void calculate_IMU_error() {
  ...
  /*
   * Don't worry too much about what this is doing. The error values it computes
   * are applied to the raw gyro and accelerometer values AccX, AccY, AccZ,
   * GyroX, GyroY, GyroZ in getIMUdata(). This eliminates drift in the
   * measurement.
   */
  int16_t AcX, AcY, AcZ, GyX, GyY, GyZ, MgX, MgY, MgZ;
  AccErrorX = 0.0;
...
void Madgwick(float gx, float gy, float gz, float ax, float ay, float az,
              float mx, float my, float mz, float invSampleFreq) {
...
void setDesStateSerial(int controlledAxis) {
  ...
  /*
     ...
     descr
                  ===     					====
     ===== controlledAxis 		int 		The axis about which the
     user's serial inputs set the desired angle. 1: roll 2: pitch
  */
  if (Serial.available()) {
  ...
  if (useSineWave) {
    ...
    desiredAngle = 10 * sin(2 * PI * sineFrequency *
                            sineTime); // Set the output to be a sin wave
...
void performSineSweep(int controlledAxis) {
  // DESCRIPTION: Performs a sine sweep from minFreq (Hz) to maxFreq (Hz) over
  // sweepTime (seconds)
  /*
          Input:
                  var 							type
     descr
                  ===     					====
     ===== controlledAxis 		int 		The axis about which the
     user's serial inputs set the desired angle. 1: roll 2: pitch minFreq
     float 	The starting frequency for the sine sweep in Hz maxFreq
     float   The ending frequency for the sine sweep in Hz sweepTime
     float 	The time period to perform the sweep over
  */
  float desiredAngle = 0;
  ...
  // }
  // if (sweepFlag){
  desiredAngle = amp * sin(PI * (maxFreq - minFreq) / pow(sweepTime, 2) *
                               pow(sineTime, 3) +
                           2 * PI * minFreq * sineTime);
...
void getDesState() {
  ...
  roll_des =
      constrain(roll_des, -1.0, 1.0) * maxRoll; // Between -maxRoll and +maxRoll
  pitch_des = constrain(pitch_des, -1.0, 1.0) *
              maxPitch; // Between -maxPitch and +maxPitch
  yaw_des =
      constrain(yaw_des, -1.0, 1.0) * maxYaw; // Between -maxYaw and +maxYaw
...
void controlANGLE() {
  ...
  integral_roll = constrain(
      integral_roll, -i_limit,
  ...
  integral_yaw = constrain(
      integral_yaw, -i_limit,
      i_limit); // Saturate integrator to prevent unsafe buildup
  ...
  yaw_PID =
      0.01 *
      (Kp_yaw * pScaleYaw * error_yaw + Ki_yaw * iScaleYaw * integral_yaw +
       Kd_yaw * dScaleYaw *
           derivative_yaw); // Scaled by .01 to bring within -1 to 1 range
...
void controlANGLE2() {
  ...
  roll_des_ol =
      Kp_roll_angle * error_roll +
      Ki_roll_angle * integral_roll_ol; // - Kd_roll_angle*derivative_roll;
  ...
  pitch_des_ol =
      ...
      Ki_pitch_angle * integral_pitch_ol; // - Kd_pitch_angle*derivative_pitch;
  ...
  roll_PID =
      .01 *
      (Kp_roll_rate * error_roll + Ki_roll_rate * integral_roll_il +
       Kd_roll_rate *
           derivative_roll); // Scaled by .01 to bring within -1 to 1 range
  ...
  pitch_PID =
      .01 *
      (Kp_pitch_rate * error_pitch + Ki_pitch_rate * integral_pitch_il +
       Kd_pitch_rate *
           derivative_pitch); // Scaled by .01 to bring within -1 to 1 range
...
void controlRATE() {
  ...
  roll_PID =
      .01 *
      (Kp_roll_rate * error_roll + Ki_roll_rate * integral_roll +
       Kd_roll_rate *
           derivative_roll); // Scaled by .01 to bring within -1 to 1 range
  ...
  pitch_PID =
      .01 *
      (Kp_pitch_rate * error_pitch + Ki_pitch_rate * integral_pitch +
       Kd_pitch_rate *
           derivative_pitch); // Scaled by .01 to bring within -1 to 1 range
...
void commandMotors() {
   ...
   * pulses to the ESCs (mXPin). The pulselengths being sent are mX_command_PWM,
   * computed in scaleCommands(). This may be replaced by something more
  ...
  while (wentLow <
         6) { // Keep going until final (6th) pulse is finished, then done
    timer = micros();
...
void calibrateESCs() {
  ...
  while (true) {
    ...
    Madgwick(GyroX, -GyroY, -GyroZ, -AccX, AccY, AccZ, MagY, -MagX, MagZ,
             dt);  // Updates roll_IMU, pitch_IMU, and yaw_IMU (degrees)
    getDesState(); // Convert raw commands to normalized values based on
                   // saturated control limits
    ...
    // throttleCut(); //Directly sets motor commands to low based on state of
    // ch5
    ...
    // commandMotors(); //Sends command pulses to each motor pin using
    // OneShot125 protocol
...
void getJoyAngle() {
  ...
  alpha = (static_cast<float>(alphaCounts) - fullRange_alpha_half -
           alphaCounts_min) /
              fullRange_alpha * (alpha_min - alpha_max) +
          alphaOffset;
  beta =
      (static_cast<float>(betaCounts) - fullRange_beta_half - betaCounts_min) /
          fullRange_beta * (beta_min - beta_max) +
      betaOffset;
